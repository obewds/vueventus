...
// node_modules/@thebespokepixel/es-tinycolor/esm/index.js
var mathRound = Math.round;
var mathMin = Math.min;
var mathMax = Math.max;
var isOnePointZero = (n) => typeof n === "string" && n.includes(".") && Number.parseFloat(n) === 1;
var isPercentage = (n) => typeof n === "string" && n.includes("%");
var roundIf01 = (n) => n < 1 ? mathRound(n) : n;
var roundAlpha = (a) => mathRound(100 * a) / 100;
var boundAlpha = (a) => {
  a = Number.parseFloat(a);
  return Number.isNaN(a) || a < 0 || a > 1 ? 1 : a;
};
var hasAlpha = (rgba) => rgba.a < 1 && rgba.a >= 0;
var clamp01 = (value) => mathMin(1, mathMax(0, value));
var pad2 = (c) => c.length === 1 ? `0${c}` : `${c}`;
var CSS_INTEGER = "[-\\+]?\\d+%?";
var CSS_NUMBER = "[-\\+]?\\d*\\.\\d+%?";
var CSS_UNIT = `(?:${CSS_NUMBER})|(?:${CSS_INTEGER})`;
var isValidCSSUnit = (color) => new RegExp(CSS_UNIT).test(color);
var isValidCSSUnitRGB = (rgb) => isValidCSSUnit(rgb.r) && isValidCSSUnit(rgb.g) && isValidCSSUnit(rgb.b);
...
var convertHexToInt = (value) => Number.parseInt(value, 16);
var convertHexToDecimal = (h) => convertHexToInt(h) / 255;
var convertToPercentage = (n) => n <= 1 ? `${n * 100}%` : n;
var rawToRgba = (raw) => {
  const [r, g, b] = [raw._r, raw._g, raw._b].map((value) => mathRound(value));
  return { r, g, b, a: raw._roundA };
};
var rawToDeepRgba = (raw) => ({ r: raw._r, g: raw._g, b: raw._b, a: raw._a });
var conformRgba = (rgba) => {
  const [r, g, b] = [rgba.r, rgba.g, rgba.b].map((n) => bound01(n, 255) * 255);
  return { r, g, b, a: boundAlpha(rgba.a) };
};
var rgbaToPercentageRgba = (rgba) => {
  const [r, g, b] = [rgba.r, rgba.g, rgba.b].map((n) => `${mathRound(bound01(n, 255) * 100)}%`);
  return { r, g, b, a: rgba.a };
};
var rgbaToString = (rgba) => rgba.a === 1 ? `rgb(${rgba.r}, ${rgba.g}, ${rgba.b})` : `rgba(${rgba.r}, ${rgba.g}, ${rgba.b}, ${rgba.a})`;
var rgbaToArray = (rgba) => rgba.a === 1 ? [rgba.r, rgba.g, rgba.b] : [rgba.r, rgba.g, rgba.b, mathRound(rgba.a * 255)];
var rgbaToHex = (rgba, allowShort) => {
...
var rgbToHex = (rgba, allowShort) => rgbaToHex({ ...rgba, a: 1 }, allowShort);
var calcBrightness = (rgb) => (rgb.r * 299 + rgb.g * 587 + rgb.b * 114) / 1e3;
...
var TinyColorExtension = class {
  constructor(api2, id, options = {}) {
    this.api = api2;
    this.id = id;
...
var TinyColorExtensionAPI = class {
  constructor() {
    this.colorspaces = {};
    this.options = {
      alphaFormat: "rgb",
...
var api$6 = TinyColor.registerFormat("rgb");
api$6.shouldHandleInput = (input) => typeof input === "object" && isValidCSSUnitRGB(input) && !isPercentage(input.r) || rgbStringToObject(input);
api$6.toRgb = (input) => typeof input === "object" && conformRgba(input) || conformRgba(rgbStringToObject(input));
api$6.toRaw = (rgba) => rgba;
api$6.toString = (rgba) => rgbaToString(rgba);
var api$5 = TinyColor.registerFormat("prgb");
...
var api$4 = TinyColor.registerFormat("hex", {
  alias: ["hex3", "hex6"]
});
...
var hexToString$1 = (rgba, short = api$4.options.shortHex) => `#${api$4.options.upperCaseHex ? rgbToHex(rgba, short).toUpperCase() : rgbToHex(rgba, short)}`;
...
api$4.toString = (rgba) => {
  ...
  if (hasAlpha(rgba)) {
    return api$4.options.alphaFormat === "hex" ? hexToString$1(rgba) : api$4.print(api$4.options.alphaFormat, rgba);
  }
  return hexToString$1(rgba);
};
...
var hexToString = (rgba, short = api$3.options.shortHex) => `#${api$3.options.upperCaseHex ? rgbaToHex(rgba, short).toUpperCase() : rgbaToHex(rgba, short)}`;
...
api$3.toString = (rgba) => {
  ...
  if (hasAlpha(rgba)) {
    return api$3.options.alphaFormat === "hex" ? hexToString(rgba) : api$3.print(api$3.options.alphaFormat, rgba);
  }
  return hexToString(rgba);
};
var api$2 = TinyColor.registerFormat("hsl");
...
var isValidCSSUnitHSL = (hsl) => isValidCSSUnit(hsl.h) && isValidCSSUnit(hsl.s) && isValidCSSUnit(hsl.l);
...
var api$1 = TinyColor.registerFormat("hsv");
...
var isValidCSSUnitHSV = (hsv) => isValidCSSUnit(hsv.h) && isValidCSSUnit(hsv.s) && isValidCSSUnit(hsv.v);
...
var api = TinyColor.registerFormat("name", {
  alias: ["toName"]
});
function flip(o) {
  const flipped = {};
...
//# sourceMappingURL=@thebespokepixel_es-tinycolor.js.map
